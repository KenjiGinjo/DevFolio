...
const linksStr = `<link rel="apple-touch-startup-image" media="screen and (device-width: 430px) and (device-height: 932px) and (-webkit-device-pixel-ratio: 3) and (orientation: landscape)" href="splash_screens/iPhone_15_Pro_Max__iPhone_15_Plus__iPhone_14_Pro_Max_landscape.png">
...
<link rel="apple-touch-startup-image" media="screen and (device-width: 744px) and (device-height: 1133px) and (-webkit-device-pixel-ratio: 2) and (orientation: portrait)" href="splash_screens/8.3__iPad_Mini_portrait.png"></link>`;

export const appleTouchStartupImages = linksStr.split("\n").map((link) => {
  const matches = link.match(/media="([^"]+)" href="([^"]+)"/);
  if (matches) {
    const [_, media, url] = matches;
    return {
      media: media,
